...
- Domain: none. `agent` has no business it belongs to.
...
Touch the machine. That capability lives on a different branch of this
platform.

## design notes

The parent every role-defined agent extends. `agent` has no business it
belongs to; that is what makes it the parent rather than a peer of
`sales-agent`.

Terminal access and filesystem reads live in `operator-agent`, a sibling
branch, because inheritance here is additive: a capability placed on this
role would be inherited by every sales and support agent beneath it, and no
