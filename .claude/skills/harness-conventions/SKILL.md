...
- Runtimes are built **once at startup** in `main.py:lifespan` and cached in `app.state.runtimes` keyed by runtime id: the opaque ids `create_app(agents=...)` registers, or, without `agents=`, the ids `AGENT_REGISTRATIONS` maps to `"{role}"` / `"{role}@{client}"`. Nothing parses an id. Resolve, don't rebuild.
