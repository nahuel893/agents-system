...
| Salidas | `columns` (lista), `rows` (lista de listas, apta para JSON; los importes como strings), `row_count`, `truncated` (coincidieron más filas que `row_limit`), `row_limit`, `empty_result`, `relations` (vistas leídas). |
