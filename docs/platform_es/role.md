...
| `audit_policy` | `object` | obligatorio | Qué eventos debe guardar el runtime en el registro de auditoría. Subcampos: `log_tool_calls` (booleano), `log_delegations` (booleano), `log_escalations` (booleano), `retention_days` (entero o `null` para el valor por defecto de la plataforma), `capture_tool_input` (booleano; `true` guarda los textos libres sin redactar), `redact_keys` (lista de claves extra del payload a redactar). |

### Herencia: aditiva para la capacidad, sustractiva para la seguridad

`extends` suma capacidad: un hijo conserva las herramientas, los permisos y la prosa de su padre, y agrega los suyos. La seguridad funciona al revés. Un hijo puede igualar o endurecer el `autonomy`, los `execution_limits` y la redacción de auditoría de su padre, y nunca aflojarlos ni subirlos (`platform/roles/base/policy.md`).
...
- **Límites de ejecución.** Un límite ausente o `null` significa el valor por defecto de la plataforma (`tool_call_timeout_s` 10, `total_execution_timeout_s` 60, `max_tool_calls` 20, `max_delegation_depth` 2, `max_clarification_attempts` 3). Vale para los dos lados. Por eso **un agente del importador no puede subir un límite de ejecución por encima del valor de su padre, ni por encima del valor por defecto de la plataforma cuando el padre no fija ninguno.** Tampoco puede escribir `null` para deshacer un límite que su padre endureció. Un límite tiene que ser un número finito y no negativo: se rechazan `NaN`, infinito, los valores negativos y los booleanos. `0` está permitido.
- **Redacción de auditoría.** Un agente del importador no puede poner `audit_policy.capture_tool_input` en un valor verdadero salvo que su padre ya lo tenga. Cuenta cualquier valor verdadero, porque así lo lee el redactor. Si declara una lista `redact_keys`, tiene que ser una lista de strings y conservar cada clave que su padre redacta.
- **Condiciones de escalamiento.** Las `escalation_rules.conditions` de un agente del importador se suman a las de su padre; nunca las reemplazan. Así, `conditions: []` conserva la lista del padre, y una condición nueva se agrega después de ella.
- **Sin padre.** Un agente de carpeta sin `extends:` tiene como techo los valores por defecto de la plataforma: `supervised`, los límites de arriba y la redacción activada. Omitir `extends` no es una forma de esquivar la regla.
...
Los pliegues entre dos roles predefinidos no se validan así, porque la plataforma escribe los dos archivos (`data-agent` y `summary-agent` corren en `full` bajo una cadena `supervised`, y un rol predefinido reescribe su lista `conditions` completa). Las sobreescrituras de despliegue mantienen su propia validación sustractiva contra el rol resuelto (`docs/platform_es/deployment.md`).

Un `Agent` es inmutable en profundidad. Al construirse copia cada lista y cada diccionario que recibe a uno inmutable, así que cambiar los originales después no cambia lo que resuelve, tampoco cuando es el `extends=` de otro `Agent`. Igual se puede serializar con `pickle`, copiar con `copy.deepcopy` y pasar a `dataclasses.asdict`: sus mapeos son `dict` de solo lectura.

Los parámetros de `Agent(...)`, y las sobreescrituras que recibe `Agent.from_folder(...)`, se validan al construir el `Agent`, con las mismas reglas que el loader aplica a esos campos en YAML:

- `untrusted_input` tiene que ser `True`, `False` u omitirse. Se rechaza un string como `'false'`, un número o una sobreescritura explícita con `None`.
- `tools`, `skills` y `permissions` reciben una lista de strings. Un string solo cuenta como una lista de un elemento.
- `permissions` también acepta `"inherit"` o una directiva, que se resuelve igual que en un manifiesto: `{"inherit": True, "add": [...], "remove": [...]}` o `{"override": [...]}`. Cualquier otro mapeo se rechaza, porque el loader lo resolvería al conjunto completo del padre.
- Los campos de política (`context`, `escalation_rules`, `delegation_policy`, `memory_policy`, `audit_policy`) tienen que ser mapeos, y `skill_contents` tiene que mapear nombres de habilidad a su texto.
- Cada clave de `skill_contents` tiene que estar en `skills`. En `from_folder` sin sobreescritura `skills=`, decide la lista propia de la carpeta, así que esto se valida al resolver el agente.
...
`_parse_escalation_descriptions` omite por completo un bloque de código con cercas (una línea que, sin espacios, empieza con ` ``` `, con o sin etiqueta de lenguaje) mientras está abierto, de modo que una viñeta `` - `nombre` — texto `` usada como ejemplo de documentación para un futuro mantenedor nunca se confunde con una descripción real. Fuera de cualquier cerca, el mismo nombre de condición puede tener como máximo una viñeta de descripción por archivo — una segunda lanza `DefinitionError` nombrando la condición, en lugar de que la última ocurrencia gane en silencio. Una cerca abierta y nunca cerrada en ninguna parte del archivo también lanza `DefinitionError`, nombrando el archivo, en lugar de descartar en silencio toda viñeta real posterior a la cerca sin cerrar.

El NOMBRE de una condición en `escalation_rules.conditions` se normaliza igual que la prosa de una descripción: las corridas de espacios internos (un doble espacio o un tabulador de más) se colapsan a uno solo. Un salto de línea incrustado no es un formato descuidado para prolijar, sin embargo — es un identificador corrupto, así que `harness/factory.py::_compose_prompt` lanza `FactoryError` en lugar de colapsarlo en silencio. Como una descripción se busca por este nombre normalizado, las claves propias de `descriptions` se normalizan de la misma manera antes de la búsqueda — una descripción indexada por el nombre de condición crudo (sin normalizar) de un rol, un deployment o un importador igual se encuentra, en lugar de caer en silencio al render de nombre desnudo.
