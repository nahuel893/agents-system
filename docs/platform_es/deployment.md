...
deployments/
  acme/
    sales-agent/          ← sobreescritura de ACME: preventista + DeW + español coloquial AR
      role.md
      manifest.md
      policy.md
      skills/
...
### `deployments/acme/sales-agent/role.md` (sobreescritura)
```
...
purpose_extension: >
  ...
  coloquiales de productos argentinos, contrastarlos con el catálogo de ACME
...
- **`clients`** vale solo para un rol predefinido registrado por nombre. Hace fallar el arranque para una entrada `Agent`, para un id que `agents` no registra, y sin `agents`: un cliente nombra una sobreescritura sustractiva, y una ignorada en silencio serviría al agente sin su acotamiento. Una sobreescritura de cliente necesita un `RootConfig(deployments_root=...)` explícito.
...
- Alcance de entrega de ACME: `docs/delivery/acme-seller-ai.md`
