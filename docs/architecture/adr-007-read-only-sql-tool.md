...
| Text crafted to make validation itself slow (a long `AND`/`OR` chain) | Linear checks: a function's name is read from the one render of the statement, never by rendering each node on its own (which made a boolean chain quadratic: 30 s at the length cap, on the event loop); a 2,500-node budget on the parsed tree; the guard runs in a worker thread, so the event loop and the harness's per-call timeout stay in control | A thread cannot be killed: after a timeout the guard still finishes, but its work is bounded (well under a second at the length cap) |
