...
| Cambios de grants después del arranque (alguien otorga `INSERT`, o `SELECT` sobre otra vista) | `check_query_role` en cada llamada: la herramienta se niega a ejecutar | — |
...
Valores por defecto: 100 filas por llamada (como máximo el `HARD_ROW_CEILING` de la plataforma, 500) y un timeout de sentencia de 5 segundos (como máximo 30 s); manténgalo por debajo del timeout por llamada a herramienta del harness.
