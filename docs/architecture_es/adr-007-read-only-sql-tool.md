...
| Texto armado para que la propia validación sea lenta o falle: `ARRAY[...]` anidados (el parser de sqlglot duplica su trabajo por nivel: 33 s con 141 caracteres), cadenas de subíndices (cuadráticas), cadenas largas de `AND`/`OR`, anidamiento profundo que agota el límite de recursión de Python | Límites chequeados sobre la secuencia de tokens antes de que corra el parser (1.000 tokens, 20 niveles de anidamiento, 4 niveles abiertos por una palabra clave de tipo); un presupuesto del parser de 2.500 nodos que incluye los intentos abandonados; un límite de 80 niveles para el árbol; chequeos lineales después del parseo (el nombre de una función se lee del único render de la sentencia, nunca renderizando cada nodo por separado, lo que volvía cuadrática una cadena booleana); todo fallo del guard es un rechazo fijo, nunca una excepción que aborte el turno. Un test de benchmark ejecuta los peores casos en los límites (anidamiento tipado y simple, cadenas de subíndices y unarias, cadenas booleanas, subconsultas, CTEs, joins, uniones, `CASE`) y cada uno debe terminar en menos de 200 ms | La cota es medida, no demostrada: descansa en los límites y en el parser de sqlglot en la versión fijada. Un hilo no se puede matar, así que un guard que sí tardara seguiría corriendo después del timeout del harness; corre en su propio executor pequeño, por lo que solo podría demorar otras llamadas a `sql_query`, nunca el executor por defecto del loop |
