...
`WHATSAPP_RUNTIME_ID` se busca como una clave opaca en el registro (ADR-004) — `create_app(agents=...)`, o `AGENT_REGISTRATIONS` cuando la aplicación no pasa `agents=` —, nunca se parsea. Un id que ninguna entrada registra hace fallar el arranque, nombrándolo, digan lo que digan las credenciales; el control `untrusted_input` de ADR-002 C.13 y el presupuesto del lease de arriba se aplican sin cambios al agente registrado, incluido un `Agent` propio.
