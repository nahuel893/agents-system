...
| D-041 | `AuditSink.stop()` must await the drainer instead of cancelling it | D-027 | ~40 | **Done — #8.** `stop()` now awaits the drainer's own shutdown-drain tail (bounded by a `timeout`, default 5s, that falls back to cancelling only if the drainer is wedged) instead of cancelling it immediately. The strict xfail in `tests/test_audit_wiring.py` was removed and the test passes for real. |
| D-042 | `_seq_counter` prune strategy, and a sequence source that survives N workers | D-029 | ~120 | **Done — #9.** The in-process counter is gone; `_allocate_sequence`/`_seq_counter` were removed from `audit/recorder.py`. The sequence is now allocated atomically per `correlation_id` against a dedicated `audit_sequence` table (migration `005_audit_sequence`), inside the same transaction `AuditSink._flush_batch` already commits with — which survives any number of worker processes by construction. |
