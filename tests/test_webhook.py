...
from acme.config import Settings, get_settings
...
def make_settings(**overrides: str) -> Settings:
    ...
    defaults = dict(
        ...
        database_url="postgresql+asyncpg://localhost:5432/acme_test",
...
def test_verify_signature_valid() -> None:
    ...
    from acme.integration.meta_signature import verify_signature
...
async def test_post_duplicate_message(
    ...
    with patch("acme.integration.webhook.get_redis_client", return_value=mock_redis):
        response = await client.post(
            "/webhook",
            content=text_payload,
            headers={
...
async def test_post_unregistered_client(
    ...
    with (
        patch("acme.integration.webhook.get_redis_client", return_value=mock_redis),
        patch("acme.integration.webhook.lookup_or_create_client", mock_lookup),
    ):
        response = await client.post(
            "/webhook",
            content=text_payload,
