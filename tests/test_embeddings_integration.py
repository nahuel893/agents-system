...
from agents_system.services.embeddings import LocalBGEEmbeddingProvider
...
async def test_local_provider_real_bge_m3_semantic() -> None:
    ...
    branda, branda_variant, unrelated = await provider.embed(
        [
            "BrandA Cristal 1L retornable",
            "cerveza BrandA un litro",
            "zapatillas Adidas running negras",
        ]
    )

    sim_related = _cosine(branda, branda_variant)
    sim_unrelated = _cosine(branda, unrelated)

    # BrandA ↔ BrandA-variant must be MORE similar than BrandA ↔ zapatillas
    assert sim_related > sim_unrelated, (
        f"Expected related products to be closer: "
        f"sim(branda, variant)={sim_related:.4f} vs "
        f"sim(branda, unrelated)={sim_unrelated:.4f}"
    )
