...
from agents_system.services.embeddings import (
    EmbeddingProvider,
...
async def test_fake_provider_returns_correct_dimensions() -> None:
    ...
    vectors = await provider.embed(["BrandA 1L"])
...
async def test_fake_provider_deterministic() -> None:
    ...
    provider = FakeEmbeddingProvider(dimensions=512)
    v1 = await provider.embed(["BrandA 1L"])
    v2 = await provider.embed(["BrandA 1L"])
    assert v1 == v2
...
async def test_fake_provider_different_inputs_different_vectors() -> None:
    ...
    provider = FakeEmbeddingProvider(dimensions=512)
    [v1] = await provider.embed(["BrandA 1L"])
    [v2] = await provider.embed(["BrandB lata"])
    assert v1 != v2
...
async def test_fake_provider_batch() -> None:
    ...
    provider = FakeEmbeddingProvider(dimensions=512)
    texts = ["BrandA 1L", "BrandB lata", "Stella retornable"]
...
@patch("agents_system.services.embeddings.SentenceTransformer")
async def test_local_provider_truncates_to_dimensions(mock_st: MagicMock) -> None:
    ...
    vectors = await provider.embed(["BrandA 1L", "BrandB lata"])
