...
class ScenarioResult:
    ...
    def total_usage(self) -> TurnUsage | None:
        ...
        that returned one (#78 Phase 0) -- `None` when every run raised
        before completing a turn (`RunOutcome.usage` is `None` on all of
        them)."""
        known = [run.usage for run in self.runs if run.usage is not None]
        if not known:
            return None
        return _sum_turn_usage(known)
    ...
    *,
    ...
    try:
        ...
        for index in range(runs):
            ...
            # #78 Phase 0 -- one TurnUsage per turn this run makes; summed
            # into the RunOutcome below. Read from a freshly-typed
            # `turn_result` (not `history`, which stays annotated
            # `list[AnyMessage]`) so `.usage` type-checks.
