...
3. **Grain.** One row per sale in `agentsys_sales`, one row per line in
   `agentsys_sale_items`. A schema that stores sales at line grain
...
_TOP_CUSTOMERS_SQL = text(
    ...
    SELECT
        ...
        COALESCE(SUM(s.amount), 0) AS revenue
    FROM agentsys_customers c
    JOIN agentsys_sales s ON s.customer_id = c.customer_id
    WHERE s.status IN :statuses
      AND s.sold_at >= :since
...
_TOP_PRODUCTS_SQL = text(
    ...
    FROM agentsys_sale_items i
    JOIN agentsys_sales s ON s.sale_id = i.sale_id
    WHERE s.status IN :statuses
      AND s.sold_at >= :since
    GROUP BY i.sku
    ORDER BY revenue DESC
