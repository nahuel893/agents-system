...
| D-047 | CI runs only 2 of the 8 files containing integration tests | — | medium | — | — | todo | `pyproject.toml:74` sets `addopts = "-m 'not integration'"`; the main `ci` job runs bare `pytest` (`ci.yml:30`). Only `test_reports_integration.py` and `test_audit_migration_integration.py` get dedicated `-m integration` jobs. `test_platform_tools_integration.py`, `test_openai_compatible_integration.py`, `test_embeddings_integration.py`, `test_acme_reports.py`, `test_reports.py` and `test_db_integration.py` never execute in CI. Same blind-spot class `ci.yml:32-37` already documents having caused the BI read-only gap. |
