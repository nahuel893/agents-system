...
|----|-----------------|-------|----|--------|-----------|--------|--------|
| D-007-pr1 | Alembic infra + `AuditEvent` + table-ownership rule + partition lifecycle | claude-code | high | `feat/D-007-pr1-schema-model-migration` | — | done | 549 unit + 5 integration green, ruff + mypy clean. Fixed 4 defects beyond the original scope — see task details. 1282 lines, **size decision pending**. PR #26 merged 2026-08-22. |
| D-026 | Audit events + redactor (Pydantic union, PII default-deny) | claude-code | high | `feat/D-007-pr2-events-redactor` | D-007-pr1 | done | 606 green, ruff + mypy clean. 1711 lines over pr1. Found the `client`/`deployment` mock divergence: every event recorded `"unknown"` as the deployment. Two strict xfails left as forcing functions for pr3. PR #27 merged 2026-08-22. |
| D-027 | Audit sink wiring (injector, interceptor, factory, graph) | claude-code | high | `feat/D-007-pr3-sink-wiring` | D-026 | done | 634 green, ruff + mypy clean on 52 files. 1122 lines over pr2. Fixed 4 chained silent defects that made the whole feature inert — see task details. `AuditSink.stop()` tail loss pinned as strict xfail. PR #28 merged 2026-08-22. |
...
| D-042 | `_seq_counter` grows without bound and is per-process | — | medium | — | D-027 | todo | Module-level dict in `audit/recorder.py`, one entry per `correlation_id`, never pruned. Also per-worker: every process counts the `"none"` fallback correlation from 1, so two workers emitting a contextless event in the same microsecond violate `uq_audit_event_correlation_sequence` and lose the whole batch. Only the test suite calls `.clear()`, which is why the suite cannot see it. **D-029 is now decided, so the direction is fixed**: since Stage B (100 concurrent, N processes) is a real target, the sequence must move to the DATABASE — a per-correlation sequence or an INSERT-time expression. A smarter in-process counter or per-request cleanup does not survive N workers and is therefore not an acceptable fix. |
| D-043 | `audit_event` never persisted: ORM column type diverged from the migration | claude-code | high | — | D-007-pr1 | in_review | FIXED on `fix/D-043-audit-tz-mismatch` (7087fbb). Migration creates `occurred_at TIMESTAMPTZ`; the model annotated it as a bare `Mapped[datetime]`, so SQLAlchemy inferred `DateTime(timezone=False)` and every INSERT compiled with a `::TIMESTAMP WITHOUT TIME ZONE` cast. asyncpg rejected every batch; the drainer logged and dropped. Audit recorded NOTHING against real PostgreSQL. Same ORM/DDL divergence class `models/base.py` warns about for partitioning. Invisible to all 634 tests (unit suite mocks the session; migration tests assert DDL, never drive an ORM INSERT). Fix: `DateTime(timezone=True)` + a metadata assertion + the missing ORM round-trip integration test. Verified: 0 rows before, 10 rows in `audit_event_2026_08` after, 0 drain failures. Only visible because 08e82ee bound the exception in the failure log. |
| D-044 | Production registry serves hardcoded stubs as real connectors | — | high | — | — | todo | **Blocks `sales-agent` shipping to clients.** `main.py:239` builds the only runtime registry via `build_acme_rag_registry`, and `rag_connector.py:20-25` imports `order_writer`, `client_lookup`, `message_sender`, `session_state` straight from `connectors/stubs.py`. `order_writer` returns a fabricated `ord-NNNN` with totals from a hardcoded price dict and NEVER writes to `orders`/`order_items` — a real customer is told an order exists when it does not. `client_lookup` matches a 2-phone dict while the `clients` table holds 32 rows; note `webhook.py` separately calls the real `lookup_or_create_client`, so the surface the model can invoke is disconnected from the real registry. `message_sender` returns `status: sent` with no network call (the real send happens at `webhook.py:210`, bypassing the tool). `catalog_search` and `run_report` ARE real — `run_report` is the correct pattern: it fails closed to "unavailable" instead of fabricating. Registering without error is not the same as being implemented. |
| D-045 | Per-tenant isolation in the vector catalog before onboarding a 2nd client | — | medium | — | — | todo | `catalog_embeddings` (`models/tables.py:120-134`) has no tenant column; `services/catalog.py:38-46` filters only on `active.is_(True)`. Single-tenant by design today (one ACME catalog) so no live leak, but the moment a second client's catalog lands in that table either the global unique `sku` collides or client A's semantic search returns client B's products — zero code change required to trigger. Needs `client_id` + `unique(client_id, sku)` + a mandatory tenant predicate on `search_vector` and `search_keywords`. |
| D-046 | `/health` reports ok while the LLM provider is unreachable; `dropped_count` is never exposed | — | medium | — | — | todo | `main.py:397-434` probes only Postgres and Redis. A liveness probe sees green while every turn fails. Separately, `AuditSink.dropped_count` (`sink.py:51,135`) exists only as a structlog line — `rg dropped_count src/agentsys/main.py` → 0 hits, and there is no `/metrics` endpoint or OTel dependency anywhere. Both are cheap and commit to no pending decision. Proven live: `/health` returned `{"status":"ok"}` during the total audit outage of D-043. |
| D-047 | Three integration-marked test files never run in CI | — | medium | — | — | todo | `pyproject.toml:74` sets `addopts = "-m 'not integration'"` and the main `ci` job runs bare `pytest` (`ci.yml:30`), so integration-marked tests only execute where a dedicated job re-runs them with `-m integration`. Two are wired up: `test_reports_integration.py` (`ci.yml:97`) and `test_audit_migration_integration.py` (`ci.yml:149`). **Three are not: `test_db_integration.py`, `test_embeddings_integration.py`, `test_openai_compatible_integration.py`** — their tests are collected by no CI job at all. CORRECTION (found in review of #31): an earlier version of this row claimed 6 uncovered files out of 8, counted by grepping `pytest.mark.integration`. That grep matched **prose inside docstrings**. `test_platform_tools_integration.py`, `test_reports.py` and `test_acme_reports.py` carry ZERO real markers — the first says so explicitly at line 7 ("this file is intentionally NOT marked") because its stubs are deterministic and must run in the default suite. Verified by counting decorator lines, not mentions: the real figure is 2 of 5 covered, 3 uncovered. Same blind-spot class `ci.yml:32-37` documents for the BI read-only role. |
| D-048 | pre-commit hooks for hygiene, syntax and secret detection | claude-code | low | — | — | in_review | Scope (files): `.pre-commit-config.yaml`, `pyproject.toml`, plus whitespace-only normalisation across 14 files (`git diff --ignore-all-space` is empty over them). PR #32. Filed retroactively: this work started without a ledger row, which is the scope declaration other agents read — review flagged it correctly. Hooks: hygiene (trailing-whitespace markdown-aware, end-of-file-fixer excluding `.engram/`, mixed-line-ending), syntax (yaml/toml/json), repo (added-large-files at the 500KB default, merge-conflict with `--assume-in-merge`, case-conflict), security (detect-private-key), and `ruff-check --fix`. Full run 0.9s. NOT included and deliberate: pytest (needs live Postgres), mypy (50s cold, and only sees staged files while needing the whole project), ruff-format (would reformat 74 of 117 files — its own migration commit). Known gaps documented in the config rather than hidden: no `.gitattributes` so `--fix=lf` is not durable on a CRLF clone; `.engram/` tracked in git at all; CI never runs pre-commit so the hooks are local-only. |
| D-049 | Branch protection on `main` | — | high | — | — | todo | **The highest-value item in the backlog and it is configuration, not code.** Measured 2026-08-29: `GET /repos/nahuel893/agents-system/branches/main/protection` returns 404 — `main` is unprotected, so direct pushes are allowed, CI is advisory, and review is optional. Every rule in `docs/delivery/delivery-flow.md` is voluntary until this lands. Required: required status checks (`ci`, `bi-readonly`, `audit-migration`), at least one required approving review, dismiss stale approvals on new commits, and no direct pushes. Note the consequence for agents: with review required, an agent cannot merge its own work, which is the intended behaviour. |
| D-050 | PR template with the four mandatory questions | — | medium | — | — | todo | `.github/pull_request_template.md` does not exist. The delivery flow requires every PR to answer: what problem this solves and how we know it is solved; how it was verified (commands, output, or a failing test that now passes); the blast radius; and **how it is rolled back**. Today those get answered when the author remembers. A template makes the omission visible in review. |
| D-051 | CODEOWNERS to route review | — | low | — | D-049 | todo | No CODEOWNERS file. Review assignment is currently a social negotiation rather than a routing rule. Pairs with D-049: protection makes review required, CODEOWNERS decides who is required. Relevant paths: `src/agentsys/harness/**` (RBAC), `alembic/**` and `src/agentsys/models/**` (schema), `.github/**` (pipeline). |
| D-052 | SemVer, tags and CHANGELOG | — | medium | — | — | todo | `version = "0.1.0"` has not moved, 0 tags, 0 releases. Nobody can say what is deployed. We already write Conventional Commits, so the prefixes can drive the version bump and generate the CHANGELOG — the input exists, only the automation is missing. Prerequisite for any deploy that needs to be identified or rolled back to. |
| D-053 | DORA measurement | — | low | — | D-036, D-052 | todo | Deployment frequency, lead time for changes, change failure rate, time to restore. Deliberately last: measuring delivery before anything is delivered produces theatre. Becomes meaningful once D-036 (provisioning) and D-052 (versioning) exist. To find bottlenecks in the system, never to rank people. |
| D-029 | Runtime topology decision record | claude-code | medium | — | — | in_review | **ANSWERED.** Target set by the human: 10 concurrent conversations at launch, 100 as the standing goal. ADR at `docs/architecture/adr-001-runtime-topology.md` (+ `_es`). Decision is CHAINED: the embedder choice decides whether N processes are physically possible, and only then is worker count meaningful. Stage A (10) = one process, reachable with current code + D-030 + D-033. Stage B (100) = N processes, requires remote embedder (local BGE-M3 is 4.3GB per process against 15.8GB host RAM) plus D-032/D-042/D-046 first. Measured: `max_connections`=100, 3 engines x 15-connection default pool exhausts PostgreSQL at 3 processes. |
...
| D-032 | Pool sizing from settings, both engines | — | **high** | — | D-029 | todo | **Raised from low by ADR-001**: it is the binding constraint for Stage B, not an optimization. Measured: pool is unconfigured (`models/base.py:65`), so SQLAlchemy defaults to 5+10=15 per engine, and a process holds up to 3 engines (operational, BI, medallion) = 45 connections. Against `max_connections`=100 minus 3 reserved, three worker processes exhaust PostgreSQL before anything else becomes the bottleneck. Size as `max_connections` / expected processes, with headroom. |
| D-033 | Admission control — bounded concurrent turns | — | **high** | — | D-030 | todo | **Raised from medium by ADR-001: required for Stage A, not just Stage B.** Nothing in the codebase bounds concurrent turns — there is no semaphore and no limit anywhere. With no bound, 100 arriving conversations open 100 turns and exhaust the pool regardless of process count. The roadmap's "the real ceiling is 4x what you wrote" does not apply yet because nothing is written. Cheapest protection in the queue. |
...
| Attack entry vector | Router admin UI at `192.168.1.1` | Host has no public IP and UPnP shows no mappings, so it is either a static forward/DMZ rule or a LAN-internal source. Settle it **before** a real ACME credential lands here. |
| D-007-pr1 size | ~~Your call~~ resolved: merged as one PR (#26) | 1282 lines. ~240 is `alembic init` scaffolding (`alembic.ini`, `script.py.mako`, `uv.lock`) and 151 is the migration — both categories the `chained-pr` rule names as hard to split. Chain it (infra+model+ownership ≈700 / migration+partitions+CI ≈580, neither under 400) or record `size:exception`. |
...
| D-006 | ACME connector stubs — 5 sales-agent tools | claude-code | done — `src/agentsys/connectors/stubs.py`, wired from `main.py` |
...
`sqlalchemy.url = postgresql+asyncpg://postgres:postgres@localhost:5432/acme`
...
- **Read first:** `docs/platform/harness.md` (Capability Injector section), `docs/architecture/permission-model.md`, `docs/platform/tool.md`, and `src/acme/harness/loader.py` for the `AgentDefinition` shape. **Do NOT modify `loader.py`.**
- **Scope (files) — all NEW, disjoint from the loader:**
  - `src/acme/harness/registry.py`
  - `src/acme/harness/injector.py`
...
- **Acceptance criteria:**
  ...
  - [ ] Full suite green: `pytest`. `mypy src/` clean, `ruff check src/acme/harness/` clean.
...
The keystone glue: assembles `loader.resolve()` + `injector.resolve_tool_surface()` + loaded skill files into a single ready-to-run `EquippedRuntime`. Built interactively with the Lead. Serves the **agent_seller MVP** — `acme/sales-agent` now assembles end-to-end.
